...
if __name__ == '__main__':
    ...
    with open('data/logs.txt', 'r') as f:
        for line in f.readlines():
            if line[0] == 'g':
                ...
                n_eva = n_eva+1
                items = line.strip().split(' ')
                prec.append(float(items[2].split(',')[0]))
                recall.append(float(items[4].split(',')[0]))
                f1.append(float(items[6]))
    n_eva=range(n_eva)
    # ————————————————
    ...
    for i in range(len(step)):
        ...
        if i <= max(n_eva):
            writer.add_scalar('prec', prec[i], n_eva[i])
            writer.add_scalar('recall', recall[i], n_eva[i])
            writer.add_scalar('f1', f1[i], n_eva[i])
    writer.record()
